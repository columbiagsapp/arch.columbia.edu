...
<div id="node-<?php print $node->nid; ?>" class="tmpltzr-module tmpltzr-secondary-float tmpltzr-module-240<?php if($color){ print ' '.$color; } ?> tmpltzr-widget tmpltzr-upcomingeventswidget tmpltzr-fetched node<?php if ($sticky) { print ' sticky'; } ?><?php if (!$status) { print ' node-unpublished'; } ?><?php if ($color) { print ' '.$color; } ?> clearfix">
	...
	<h4 style="text-align:center;margin:0 auto 25px;">Upcoming Events</h4>
  	<?php
		...
		print '<div id="upcomingeventswidget-output"></div>' .
			...
			'</script>';
  	?>
  	<h4 style="text-align:center;margin:0 auto;text-transform:none;">
  		<?php
  			if(!empty($node->field_more_link)){
  				print $node->field_more_link[0]['view'];
  			}
  		?>
