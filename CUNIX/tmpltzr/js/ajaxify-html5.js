...
(function(window,undefined){
	...
	$(function(){
		...
		$.expr[':'].internal = function(obj, index, meta, stack){
			...
			if((url.indexOf("/admin/") >= 0) || (( copypaste == true) && (url.indexOf("/edit") >= 0)) || (url.substring(0,1) === '#') ){
		...
		*/
		...
		$.fn.ajaxify = function(){	
			...
			$(this).find('a:internal:not(#gsapplogo, .term-index-term)').click(function(event){ //exempt GSAPP Logo so it reloads everything
				safelog('anchorclick: '+anchorclick);
...
*/
		...
		$(window).bind('statechange',function(){	
			if(!anchorclick){
				...
				safelog('making anchorclick false');
				anchorclick = false;
			} // end if(!anchorclick)
