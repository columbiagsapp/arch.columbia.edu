	...
	if( $('#course-blogs-index-listing').length > 0){
		...
		if( (ww >= 1270) && (path.indexOf('/about/people') < 0) ){
			...
			if( !$('body').hasClass("front") ){
				...
				//don't want to activate second pane when editing
				if( $('#panels-ipe-control-container').length == 0 ){
					$('#tmpltzr #main .view .views-row, #tmpltzr .panel-display .panel-pane .pane-content').each(function(i){
						if($('.tmpltzr-secondary-float', this).length != 0){
							id = $('.tmpltzr-secondary-float', this).attr('id');
							console.log('id: '+id);
							$(this).addClass(id).addClass('empty');
							if( $('#tmpltzr #main').length > 0 ){
								$('#tmpltzr #right-sidebar').append($('.tmpltzr-secondary-float', this));
							}else{//panels
								$('#tmpltzr .panel-col-last').append($('.tmpltzr-secondary-float', this));
							}							
						}
					});	
				}else{
					$('#tmpltzr .panel-col-last').hide();
				}	
			}			
		}else{
			$('#wrapper').css('width', '520px');
			...
			var insertClass = '';
			if(!$('body').hasClass("front")){
				$('#tmpltzr #right-sidebar .tmpltzr-secondary-float, #tmpltzr .panel-col-last .tmpltzr-secondary-float').each(function(){
					if($('#tmpltzr #main').length > 0){
						insertClass = '#tmpltzr #main .view .views-row.' + $(this).attr('id');
					}else{//panels
						insertClass = '#tmpltzr .panel-display .panel-pane .pane-content.' + $(this).attr('id');
